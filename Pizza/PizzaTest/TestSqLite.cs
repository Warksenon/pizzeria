...
namespace PizzaTest
{
    ...
    public class TestSqLite
    {
        ...
        public void TestSaveOrderList()
        {
            ...
            TInsertAndQuestionSQL sql = new TInsertAndQuestionSQL();
            sql.RemoveAllTask();
            TestAddOrderFromForm1 test = new TestAddOrderFromForm1();
            ...
            string price = Convert.ToString(CalcuationPrice(order));
            ...
            price = Convert.ToString(CalcuationPrice(order));
            order.PriceAll.Price = price + "zł";
        ...
        public int CalcuationPrice (Order order)
        {
            AddOrderFromForm1 addOrder = new AddOrderFromForm1();
            int price = 0;
            foreach (var d in order.ListDishes)
            {
                price += addOrder.FindsPrice(d.Price);
            }
            return price;
        }
    }
